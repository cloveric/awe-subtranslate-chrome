...
### _让 YouTube 从“能看”变“秒懂”：低延迟双语字幕翻译。_
...
> _看 YouTube 更轻松：边听边看边懂，不再反复暂停回放。_
> _字幕翻译是主角，网页翻译是加成。_
...
- **YouTube 字幕翻译优先** — 实时字幕、自动字幕、已有字幕都可覆盖
- **低延迟观看体验** — 原生翻译轨优先 + 轨道预取 + 提前渲染，尽量减少“慢半拍”
- **双语同屏更高效** — 中文快速理解，英文原句同步跟读
- **开箱即用** — 免费引擎可直接用，安装后即可体验
- **网页翻译一并提供** — 网页正文一键翻译，统一入口管理

### 🧲 适合这些用户

- 经常看 YouTube 英文内容（访谈、播客、教程、新闻）
- 需要“听懂 + 看懂”，不想只看单语字幕
- 想保留英文原句做语言学习或内容复盘
- 希望字幕翻译和网页翻译都在一个插件内完成
...
- **一键开翻** — 点击播放器内 `译` 按钮即可开启字幕翻译
- **实时/自动字幕都能跟** — 默认保持 YouTube 原生 CC 关闭，必要时短暂预热字幕轨并自动恢复关闭
- **低延迟优先链路** — 优先使用 YouTube 原生翻译轨，缺失时回退缓存翻译引擎
- **重叠 cue 对齐** — 在 rollup 场景优先选择最新活跃 cue，减少慢半拍
- **字幕稳定不抢画面** — 译文持续跟随字幕区域，避免遮挡关键内容
- **播放器居中兜底** — 原生字幕节点暂不可用时，译文仍保持播放器居中定位
- **失败自动兜底** — 连续失败会自动暂停并给出清晰提示
...
> 默认不需要手动点击 CC。扩展会尽量保持 YouTube 原生 CC 为关闭状态；若需初始化字幕轨，会短暂切换后自动恢复关闭。
